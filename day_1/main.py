...
def main(input_filepath):
    ...
    part_one = np.sum(
        [(10 * get_first_digit(line, use_strings=False)) + get_last_digit(line, use_strings=False) for line in contents]
    )
    part_two = np.sum(
        [(10 * get_first_digit(line, use_strings=True)) + get_last_digit(line, use_strings=True) for line in contents]
    )
